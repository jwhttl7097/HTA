...
$(function() {
	...
	// 완료하기 버튼 클릭시 실행 <- #table-todos tbody내에 미래에 추가되는 일정에 대해서도 삭제하기 버튼에 이벤트처리가 적용되도록 함
	$("#table-todos tbody").on('click', '.btn-outline-danger', function() {
		// success의 함수안에서는 this가 어떤것인지 모르기 떄문에
		// 아래처럼 삭제할 tr을 미리 찾아서 변수에 담아놓고 다음 작업을 진행한다.
		var $tr = $(this).closest("tr");
		$.ajax({
			type: "GET",
			url: "delete",		// 실제요청 URL : http://localhost/spring-todo/todo/delete?no=100044
	...
	// 완료하기 버튼 클릭시 실행 <- #table-todos tbody내에 미래에 추가되는 일정에 대해서도 완료하기 버튼에 이벤트처리가 적용되도록 함
	$("#table-todos tbody").on('click', '.btn-outline-primary', function() {
		var $tr = $(this).closest("tr");
		$.ajax({
			type: "GET",
			url: "complete",	// 실제요청 URL : http://localhost/spring-todo/todo/complete?no=100044
			data: {no: $(this).data("todo-no")},
			success: function() {
