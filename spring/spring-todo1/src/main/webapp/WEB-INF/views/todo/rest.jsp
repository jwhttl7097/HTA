...
$(function() {	
	...
	$("#btn-post-todo").click(function() {
		...
		var requestURI = "rest/todos/"
		if (requestMethod == "PUT") {
			requestURI += $("#todo-no").val(); 
		}
	...
	$("#table-todos tbody").on('click', '.btn-link', function(event) {
		...
		$.getJSON("rest/todos/" + $(this).data("todo-no"))
			.done(function(todo) {
				$("#todo-no").val(todo.no);
	...
	// 페이지가 로딩되면 자동으로 실행된다.
	(function() {
		$.getJSON("rest/todos/", function(todos) {
			$.each(todos, function(index, todo) {
				$("#table-todos tbody").append(makeRow(todo));
			})
		});
	})();
