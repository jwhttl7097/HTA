...
public class User {
	 ...
	 * 			- new 생성자()와 같이 객체 생성할 때 만 사용할 수 있는 메소드다.
	 * 				* new 키워드로만 사용할 수 없다.
	 * 			- 생성자 메소드의 조건(객체 생성 직후에 바로 실행될 메소드의 조건)
	 * 				- 생성자 메소드의 이름은 반드시 클래스의 이름과 동일해야 한다.
	 * 				- 생성자 메소드는 반환타입이 없다.
	 */
	
	// 기본 생성자 메소드(Deefault Constructor)
	// 		- 매개변수가 하나도 없는 생성자
	// 		- 컴파일러가 자동으로 추가하는 생성자
	// 			User.java를 User.class로 변환할 떄
	// 			User.java에 정의된 생성자가 없으면 public User() {}를 자동으로 추가한다.
	public User() {
		System.out.println("기본 생성자 실행됨");
		// 객체의 초기화 작업
		// 객체가 생성되면 번호, 이름, 아이디, 비밀번호, 주소, 결혼여부 등의 값을 아래의 값으로 초기화 한다.
		no = 9999;
		name = "홍길동";
		id = "hong";
		password = "zxcv1234";
